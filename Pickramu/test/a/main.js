class TemplateConverter {
  ...
  convert(templateStr) {
    const configMatch = templateStr.match(
      /@config \[(open)\]\s*name ([^\\]+)\s*ver ([^\\]+)/
    );
    const styleMatch = templateStr.match(
      /@style \[(open)\]\s*(.+?)@style \[(close)\]/s
    );
    const contentMatches = [
      ...templateStr.matchAll(
        /@tag (\w+) \[(open)\](.+?)@tag \[\w+ \[(close)\]/gs
      ),
    ];
...
`;
    ...
    const scriptMatch = templateStr.match(
      /@script on=(\w+) \[(open)\](.+?)@script \[(close)\]/s
    );
    let scriptHtml = "";
    if (scriptMatch) {
    ...
    this.htmlContent = [htmlHeader, contentHtml, scriptHtml, "</body></html>"];
